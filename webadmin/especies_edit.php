<?php
  ...
  if ( $_POST["action"] == "update" ) {
    ...
    if ( !EsAdmin() ) {
      ...
      $query = "UPDATE pesca_especies SET ";
      if ( $ok1 ) {
        $query .= " esp_icono = '".$name1."',";
      }
      if ( $ok2 ) {
        $query .= " esp_fondo = '".$name2."',";
      }
      $query .= "esp_nombre = '".mysqli_real_escape_string($conn, ucfirst($_POST["nombre"]))."', ";
      $query .= "esp_orden = '".mysqli_real_escape_string($conn, $_POST["orden"])."', ";
      $query .= "esp_status = '".mysqli_real_escape_string($conn, $_POST["status"])."' ";
      $query .= " WHERE esp_id ='".mysqli_real_escape_string($conn, $id)."'";
      $res = @mysqli_query($conn, $query);
      if(!$res) {
        $ERR = "No se ha podido guardar la info en la base de datos actual. <br />".mysqli_error($conn);
      } else {
        $MSG = "Especie actualizada correctamente!<br />";
        ...
        foreach ( $LANGS as $k => $v ) {
          // Nombre
          if ( ExisteTextoLang( 'especies', $id, 'nombre', $v) ) {
            $qry = "UPDATE pesca_textos SET value='".mysqli_real_escape_string($conn, strip_tags($_POST["nombre_".$v], "<b><i><strong><em>"))."' WHERE referred = 'especies' AND referred_id = ".$id." AND field='nombre' AND lang = '".mysqli_real_escape_string($conn, $v)."'";
            $res = mysqli_query($conn,  $qry );
            if ( $res ) $MSG .= "Nombre ".Mayus($v).": traducción actualizada correctamente<br />";
          } else {
            $qry = "INSERT INTO pesca_textos VALUES ( '', 'especies', $id, 'nombre','".mysqli_real_escape_string($conn, $v)."','".mysqli_real_escape_string($conn, strip_tags($_POST["nombre_".$v], "<b><i><strong><em>"))."' ) ";
            $res = mysqli_query($conn,  $qry );
        ...
        $MSG .= "<br /><a href=\"especies.php\">Volver al listado de especies</a>";
        $MSG .= "<a style=\"float:right\" href=\"especies_edit.php?id=0\">Insertar nuevo registro</a>";
      }
    }
    ...
    if ( !EsAdmin() ) {
      ...
      $query = "INSERT INTO pesca_especies (esp_nombre, esp_orden, esp_status)
        VALUES (
      ...
      if(!$res) {
        ...
        $MSG = "Especie creada correctamente!<br />";
        ...
        if ( $ok1 ) {
          $query = "UPDATE pesca_especies SET esp_icono = '".mysqli_real_escape_string($conn, $name1)."' WHERE esp_id = '".mysqli_real_escape_string($conn, $id)."'";
          $result = mysqli_query($conn, $query);
          if ( !$result ) $ERR .= "Error subiendo el icono!<br />".mysqli_error($conn);
          else $MSG .= "Icono subido correctamente!<br />";
        }
        if ( $ok2 ) {
          $query = "UPDATE pesca_especies SET esp_fondo = '".mysqli_real_escape_string($conn, $name2)."' WHERE esp_id = '".mysqli_real_escape_string($conn, $id)."'";
          $result = mysqli_query($conn, $query);
  ...
  if ( $id > 0 ) {
    $trads = "";
    foreach( $LANGS AS $k => $v ) {
      $trads .= ", ( SELECT value FROM pesca_textos WHERE referred = 'especies' AND referred_id = esp_id AND field = 'nombre' AND lang = '".$v."' ) AS tra_nombre_".$v." ";
    }

    $query = "SELECT * ".$trads." FROM pesca_especies WHERE esp_id = '".mysqli_real_escape_string($conn, $id)."'";
    $res = @mysqli_query($conn, $query);
    if(!$res)	{
      $ERR .= "No se ha podido conectar con la base de datos actual.<br />".$query;
    } else if ( @mysqli_num_rows($res) == 0 ) {
      $ERR .= "La especie indicada no existe.<br /><a href=\"especies.php\">Volver al listado de especies</a>";
      $id = "0";
    } else {
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="especies.php"><i class="<?=$icon_menu?>"></i> <?=$title_menu?></a></li>
...
<?php /*                        <textarea class="form-control textarea" id="field-nombre-<?=$v?>" name="nombre_<?=$v?>" rows="4" placeholder=""><?=$item["nombre_".$v]?></textarea> */ ?>
                ...
                <div class="form-group">
                  <button type="submit" class="btn btn-primary" id="Enviar"><?=( $MODO == "add" ? "Guardar nueva especie":"Guardar cambios")?> <i class="fa fa-arrow-circle-right"></i></button> <a href="especies.php" class="btn btn-default pull-right"><i class="fa fa-arrow-circle-left"></i> Volver al listado</a>
                </div>
                <!-- /.form-group -->
