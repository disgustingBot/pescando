<?php
  ...
  if( isset($_POST["action"]) && $_POST["action"] == "delete" && $_POST["delete"] != "" ) {
    if ( EsAdmin() ) {
      ...
      $query = "DELETE FROM pesca_tipos_barcos WHERE tba_id='".mysqli_real_escape_string($conn, $info[0])."'";
      $re = @mysqli_query($conn, $query);
      if(!$re) {
        $ERR = "No se ha podido borrar el tipo de barco '".$info[1]."'.<br />".mysqli_error($conn);
      } else {
        $MSG = "Tipo de barco '".$info[1]."' borrado correctamente.";
        ...
        $q2 = "DELETE FROM pesca_textos WHERE referred = 'tipos-barcos' AND referred_id = '".mysqli_real_escape_string($conn, $info[0])."'";
      ...
      $ERR = "No tienes permisos para borrar tipos de barcos!";
    }
  }
        ...
        <div class="col-md-12">
          <div class="box">
            ...
            <div class="box-body">
              ...
              <h3 class="page-header">Listado de tipos de barcos</h3>
              ...
              <div class="table-responsive">
                <table id="main-list" class="table no-margin table-bordered table-striped">
                  ...
                  <?php
                    $q1 = "SELECT * 
                      , ( select count(*) FROM pesca_barcos WHERE bar_tipo = tba_id ) as total ";
                    foreach ( $LANGS AS $k => $v ) {
                      $q1 .= ", ( SELECT count(*) FROM pesca_textos WHERE referred = 'tipos-barcos' AND referred_id = tba_id AND field IN ('nombre','descr','pesca','barcos') AND lang = '".$v."' AND value != '' ) as ".$v;
                    }
                    $q1 .= " FROM pesca_tipos_barcos ORDER BY tba_orden";
                    $r1 = @mysqli_query($conn, $q1);
                    if ( !$r1 || mysqli_num_rows($r1) == 0 ) {
                  ?>
                  <tr>
                    <td colspan="<?=$td?>">No existen datos de ningún tipo de barcos en la base de datos.<br /><?=$q1?></td>
                  </tr>
                  <?php
                    } else {
                  ...
                  ?>
                  <tr>
                    <td style="vertical-align: middle;"><a href="tipos_barcos_edit.php?id=<?=$rw1["tba_id"]?>"><?=$rw1["tba_nombre"]?></a></td>
                    <td style="vertical-align: middle;" class="text-center">
                    <?php
                      if ( $rw1["tba_icono"] != "" && file_exists($DIR_IMG.$rw1["tba_icono"]) ) {
                        echo "<img src='".$DIR_IMG.$rw1["tba_icono"]."' style='max-height: 50px; ' alt='Icono ".$rw1["tba_nombre"]."' title='Icono ".$rw1["tba_nombre"]."' border=0 />";
                      } else {
                        echo "- No disponible -";
                      }
                    ?>
                    ...
                    <?php   } else if ( $rw1[$v] > 0 ) { ?>
                      <span class="fa fa-1g fa-check" style="color: darkorange"></span>
                    <?php   } else { ?>
                      <span class="fa fa-1g fa-times" style="color: red"></span>
                    <?php   } ?>
                    </td>
                    <?php } ?>
                    <td style="vertical-align: middle;" class="text-center"><?=$rw1["tba_orden"]?></td>
                    <td style="vertical-align: middle;" class="text-center"><?=( $rw1["tba_status"] == "A" ? "Activo" : "Inactivo" )?></td>
                    <td style="vertical-align: middle;">
                      <span class="pull-right-container">
                        <div class="pull-right">
                          <a href="tipos_barcos_edit.php?id=<?=$rw1["tba_id"]?>" class="success"><i class="fa fa-pencil-square-o"></i></a>
                        <?php if ( $rw1["total"] > 0 ) { ?>
                          <span class="text-red" data-toggle="tooltip" title="Aún existen barcos de este tipo!"><i class="fa fa-trash-o"></i></span>
                        <?php } else if ( EsAdmin() ) { ?>
                          <a href="javascript:Borrar(<?=$rw1["tba_id"]?>,'<?=$rw1["tba_nombre"]?>','<?=$rw1["tba_icono"]?>')"><i class="fa fa-trash-o"></i></a>
                        <?php } else { ?>
            ...
            <div class="box-footer">
              <div class="row">
                <div class="col-sm-6 col-xs-6">
                    <a href="tipos_barcos_edit.php?id=0"><i class="fa fa-plus"></i> Añadir nuevo tipo de barco</a>
                </div>
                <!-- /.col -->
                <div class="col-sm-6 col-xs-6">
                </div>
